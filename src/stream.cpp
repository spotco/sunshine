...
#pragma pack(pop)
  ...
  session_t *control_server_t::get_session(const net::peer_t peer, uint32_t connect_data) {
    ...
    for (auto pos = std::begin(*_sessions); pos != std::end(*_sessions); ++pos) {
      ...
      spotcobuild::note_control_connected();
      ...
      // ENet sometimes reports 0.0.0.0 as the local address on Windows; PKTINFO with
      // that source causes WSASendMsg WSAEINVAL (10022). Prefer configured external_ip.
      if (session_p->localAddress.is_unspecified() && !config::nvhttp.external_ip.empty()) {
        try {
          session_p->localAddress = boost::asio::ip::make_address(config::nvhttp.external_ip);
          BOOST_LOG(warning) << "spotcobuild: control localAddress unspecified; using external_ip="sv << config::nvhttp.external_ip;
        } catch (const boost::system::system_error &e) {
          BOOST_LOG(error) << "spotcobuild: external_ip parse failed: " << e.what();
        }
      }

      BOOST_LOG(debug) << "Control local address ["sv << session_p->localAddress.to_string() << ']';
  ...
  void recvThread(broadcast_ctx_t &ctx) {
    ...
    auto session_id_str = [](const av_session_id_t &id) -> std::string {
      if (const auto *addr = std::get_if<asio::ip::address>(&id)) {
        return addr->to_string();
      }
      if (const auto *payload = std::get_if<std::string>(&id)) {
        return util::hex_vec(*payload);
      }
      return "?";
    };
    ...
    auto populate_peer_to_session = [&]() {
      while (message_queue_queue->peek()) {
        ...
        const char *ch = (socket_type == socket_e::video) ? "VIDEO" : "AUDIO";

        switch (socket_type) {
          case socket_e::video:
            if (message_queue) {
              peer_to_video_session.emplace(session_id, message_queue);
              BOOST_LOG(info) << "spotcobuild: ping-wait REGISTER "sv << ch
                              << " key="sv << session_id_str(session_id)
                              << " map_size="sv << peer_to_video_session.size();
            } else {
              peer_to_video_session.erase(session_id);
              BOOST_LOG(info) << "spotcobuild: ping-wait UNREGISTER "sv << ch
                              << " key="sv << session_id_str(session_id)
                              << " map_size="sv << peer_to_video_session.size();
            }
            break;
          case socket_e::audio:
            if (message_queue) {
              ...
              BOOST_LOG(info) << "spotcobuild: ping-wait REGISTER "sv << ch
                              << " key="sv << session_id_str(session_id)
                              << " map_size="sv << peer_to_audio_session.size();
            } else {
    ...
    auto recv_func_init = [&](udp::socket &sock, int buf_elem, std::map<av_session_id_t, message_queue_t> &peer_to_session) {
      recv_func[buf_elem] = [&, buf_elem](const boost::system::error_code &ec, size_t bytes) {
        ...
        const bool waiting = !peer_to_session.empty();
        // Rate-limit idle unmatched spam once streams are up (map empty).
        static std::atomic<int> idle_unmatched_video {0};
        static std::atomic<int> idle_unmatched_audio {0};

        auto log_udp = [&](std::string_view disposition, std::string_view reason, std::string_view extra = {}) {
          if (!waiting && disposition != "RAISE"sv) {
            auto &counter = buf_elem ? idle_unmatched_audio : idle_unmatched_video;
            const int n = ++counter;
            if (n > 12) {
              return;
            }
            if (n == 12) {
              BOOST_LOG(warning) << "spotcobuild: suppressing further idle unmatched "sv << type_str << " UDP logs"sv;
              return;
            }
          }
          BOOST_LOG(info) << "spotcobuild: UDP "sv << type_str
                          << " from="sv << peer.address().to_string() << ':' << peer.port()
                          << " bytes="sv << bytes
                          << " waiting="sv << (waiting ? "true" : "false")
                          << " map_size="sv << peer_to_session.size()
                          << " disposition="sv << disposition
                          << " reason="sv << reason
                          << (extra.empty() ? ""sv : " "sv) << extra;
          if (disposition != "RAISE"sv && (waiting || buf_elem == 0)) {
            spotcobuild::session_timeline_t::instance().emit_active("udp_recv", nlohmann::json {
              {"channel", std::string(type_str)},
              {"bytes", bytes},
              {"from", peer.address().to_string() + ":" + std::to_string(peer.port())},
              {"disposition", std::string(disposition)},
              {"reason", std::string(reason)},
              {"map_size", peer_to_session.size()},
              {"waiting", waiting},
            });
          }
        };
        ...
        if (bytes == 4) {
          ...
          if (it != std::end(peer_to_session)) {
            log_udp("RAISE", "legacy_ping_addr_match", "payload=PING");
            BOOST_LOG(info) << "RAISE: "sv << peer.address().to_string() << ':' << peer.port() << " :: " << type_str;
            it->second->raise(peer, std::string {buf[buf_elem].data(), bytes});
            spotcobuild::note_channel_raised(buf_elem ? "audio" : "video");
          } else {
            log_udp("IGNORE", "legacy_ping_no_session_for_addr",
              std::string {"payload_hex="} + util::hex_vec(std::string {buf[buf_elem].data(), bytes}));
          }
        } else if (bytes >= sizeof(SS_PING)) {
          auto ping = (PSS_PING) buf[buf_elem].data();
          auto payload_key = std::string {ping->payload, sizeof(ping->payload)};

          // For new PING packets that include a client identifier, search by payload.
          auto it = peer_to_session.find(payload_key);
          if (it != std::end(peer_to_session)) {
            log_udp("RAISE", "v2_ping_payload_match",
              std::string {"payload_hex="} + util::hex_vec(payload_key));
            BOOST_LOG(info) << "RAISE: "sv << peer.address().to_string() << ':' << peer.port() << " :: " << type_str;
            it->second->raise(peer, std::string {buf[buf_elem].data(), bytes});
            spotcobuild::note_channel_raised(buf_elem ? "audio" : "video");
          } else {
            // Also note whether address-key would have matched (legacy path).
            const bool addr_hit = peer_to_session.find(peer.address()) != std::end(peer_to_session);
            log_udp("IGNORE", addr_hit ? "v2_payload_miss_but_addr_registered" : "v2_payload_miss_no_session",
              std::string {"payload_hex="} + util::hex_vec(payload_key) +
                " seq=" + std::to_string(util::endian::big(ping->sequenceNumber)));
          }
        } else {
          log_udp("IGNORE", "undersized_datagram",
            std::string {"payload_hex="} + util::hex_vec(std::string {buf[buf_elem].data(), bytes}));
        }
      };
    };
  ...
  int recv_ping(session_t *session, decltype(broadcast)::ptr_t ref, socket_e type, std::string_view expected_payload, udp::endpoint &peer, std::chrono::milliseconds timeout) {
    ...
    const bool legacy_addr = !(session->config.mlFeatureFlags & ML_FF_SESSION_ID_V1);

    BOOST_LOG(info) << "spotcobuild: ping-wait START channel="sv << channel
                     << " expected_payload_hex="sv << util::hex_vec(expected_payload)
                     << " mlFeatureFlags=0x"sv << util::hex(session->config.mlFeatureFlags).to_string_view()
                     << " legacy_addr_match="sv << (legacy_addr ? "true" : "false")
                     << " localAddress="sv << session->localAddress.to_string()
                     << " timeout_ms="sv << config::stream.ping_timeout.count();
    spotcobuild::session_timeline_t::instance().emit(session->diag_session_id, "ping_wait_start", nlohmann::json {
      {"channel", channel},
      {"expected_payload_hex", util::hex_vec(expected_payload)},
      {"mlFeatureFlags", session->config.mlFeatureFlags},
      {"legacy_addr_match", legacy_addr},
      {"localAddress", session->localAddress.to_string()},
      {"timeout_ms", config::stream.ping_timeout.count()},
    });
    ...
    while (current_time - start_time < config::stream.ping_timeout) {
      ...
      if (msg.find(expected_payload) != std::string::npos) {
        ...
        BOOST_LOG(info) << "spotcobuild: Received ping [v2] channel="sv << channel << " from "sv << recv_peer.address() << ':' << recv_peer.port() << " ["sv << util::hex_vec(msg) << ']';
      } else if (legacy_addr && msg == "PING"sv) {
        // Match the legacy fixed PING payload only if the new type is not supported
        BOOST_LOG(info) << "spotcobuild: Received ping [v1] channel="sv << channel << " from "sv << recv_peer.address() << ':' << recv_peer.port() << " ["sv << util::hex_vec(msg) << ']';
      } else {
        BOOST_LOG(warning) << "spotcobuild: Received non-ping on wait-queue channel="sv << channel << " from "sv << recv_peer.address() << ':' << recv_peer.port() << " ["sv << util::hex_vec(msg) << ']';
      ...
      BOOST_LOG(info) << "spotcobuild: ping-wait OK channel="sv << channel
                       << " peer="sv << peer.address().to_string() << ':' << peer.port()
                       << " localAddress="sv << session->localAddress.to_string();
      return 0;
    }

    const auto waited_ms = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - start_time).count();
    BOOST_LOG(error) << "Initial Ping Timeout channel="sv << channel << " waited_ms="sv << waited_ms
                     << " expected_payload_hex="sv << util::hex_vec(expected_payload)
                     << " localAddress="sv << session->localAddress.to_string();
    spotcobuild::session_timeline_t::instance().emit(session->diag_session_id, "ping_wait_timeout", nlohmann::json {
      {"channel", channel},
      {"waited_ms", waited_ms},
      {"expected_payload_hex", util::hex_vec(expected_payload)},
      {"localAddress", session->localAddress.to_string()},
    });
    spotcobuild::clear_busy_after_setup_failure("initial_ping_timeout");
    return -1;
  }
  ...
  namespace session {
    ...
    void join(session_t &session) {
      ...
      spotcobuild::note_setup_session_end("join_without_or_with_frame");
    }
    ...
    int start(session_t &session, const std::string &addr_string) {
      ...
      spotcobuild::note_setup_session_begin();
