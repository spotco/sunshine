...
void start_udp_probe_idle_listeners() {
  ...
  // Do not idle-bind control: ENet owns 47999 during/after sessions; fighting that bind
  // confuses Test Host UDP and can leave Windows ACLs/sockets in a bad state.
  BOOST_LOG(info) << "udp_probe idle: skipping control UDP " << control_port
                  << " (ENet-only; avoids fighting control stream / Windows bind)";
